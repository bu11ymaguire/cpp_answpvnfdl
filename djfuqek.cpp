...
{ 
  if(root == nullptr){exit(1);} //루트가 nullptr이면 종료
  TreeNode* Target=nullptr;
  TreeNode* Parents;
  bool is_left = false;
  if(root->key == k){Target = root;Parents=nullptr;}
  else
  {
    Parents = root;
    while(true)
  ...
  if(Target==nullptr){exit(1);} //아직까지 Target이 nullptr이라는 거는 search에 실패했다는 뜻이므로 종료 
  ...
  else if(Target->left!=nullptr&&Target->right!=nullptr) 
  {
    ...
    for(success=Target->right;success->left!=nullptr;)
    {
      s_parents = success;
    ...
    if(success->right == nullptr){
      ...
      if(root == Target ){root = success;Parents=nullptr;s_parents -> left = nullptr;}
      else
      {
        s_parents -> left = nullptr;
        if(is_left){Parents->left = success;}
        else{Parents->right = success;}
      }
      delete Target;
    }
    else 
    {
      ...
      success->left = Target->left;
      success->right = Target->right;
      if(root == Target){root = success;Parents=nullptr;}
      else{
        if(is_left){Parents->left = success;}
        else{Parents->right = success;}
      }
